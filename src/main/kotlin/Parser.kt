...
class Parser(data: ByteArray) {
    ...
    fun parseData(): ArrayList<TLVObject> {
        ...
        while (byteInputStream.available() > 0) {
            ...
            try {
                val bytesRead = byteInputStream.read(tagValueBuffer, 0, tagValueBuffer.size)
                if (bytesRead != tagLenToRead.toInt())
                    throw IOException("Could dot read data from stream")
            } catch (ex: Exception) {
